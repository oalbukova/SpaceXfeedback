...
var ProcessForm = function (settings) {
  this._settings = {
    ...
    attachmentsFileExt: [
      ...
      "txt",
      "doc",
    ], // дефолтные допустимые расширения для файлов
...
ProcessForm.prototype = (function () {
  ...
  var _refreshCaptcha = function (_this) {
    var captchaImg = _this._form.find(".form-captcha__image"),
  ...
  var _validateForm = function (_this) {
    ...
    if (_this._attachmentsItems.length > 0) {
      ...
      for (var i = 0, length = files.length; i < length; i++) {
        ...
        if (
          files[i].file.size >
          _this._settings.attachmentsMaxFileSize * 1024
        ) {
          _this._form
            .find('.form-attachments__item[data-id="' + i + '"]')
            .attr(
              "title",
              "Размер файла больше " +
                _this._settings.attachmentsMaxFileSize +
                "Кб"
            )
            .addClass("is-invalid");
          valid = false;
          ...
          !_validateFileExtension(
            files[i].file.name,
            _this._settings.attachmentsFileExt
          )
        ) {
          _this._form
            .find('.form-attachments__item[data-id="' + i + '"]')
            .attr("title", "Тип не соответствует разрешённым")
            .addClass("is-invalid");
          valid = false;
        } else {
          _this._form
      ...
      switch (error) {
        ...
        case "attachment":
          $.each(data[error], function (key, value) {
            _this._form
              .find(
                '.form-attachments__item[data-id="' +
                  _this._attachmentsItems[key].id +
                  '"]'
              )
              .attr("title", value)
              .addClass("is-invalid");
  ...
  var _setupListener = function (_this) {
    $(document).on(
      "change",
      _this._settings.selector + ' [name="agree"]',
      function () {
    ...
    $(document).on(
      "click",
      _this._settings.selector + " .form-captcha__refresh",
      function (e) {
        e.preventDefault();
        _refreshCaptcha(_this);
      }
    ...
    $(document).on(
      "click",
      '[data-target="' + _this._settings.selector + '"]',
      function (e) {
        e.preventDefault();
        _showForm(_this);
      }
    ...
    if (_this._isAttachmentsSection) {
      ...
      $(document).on(
        "click",
        _this._settings.selector + " .form-attachments__item-link",
        function () {
          var link = $(this),
            fileId = +link.attr("data-id"),
            file = link.closest(".form-attachments__item");
          for (
            var i = 0, length = _this._attachmentsItems.length;
            i < length;
            i++
          ) {
      ...
      $(document).on(
        "change",
        _this._settings.selector + ' input[name="attachment[]"]',
        function (e) {
