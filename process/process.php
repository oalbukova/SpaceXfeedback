...
ALLOWED_EXTENSIONS = array('jpg', 'jpeg', 'bmp', 'gif', 'png', 'txt', 'svg', 'doc'), // разрешённые расширения файлов
MAIL_FROM = 'o.albukova@yandex.ru', // от какого email будет отправляться письмо
MAIL_FROM_NAME = 'SpaceX', // от какого имени будет отправляться письмо
MAIL_SUBJECT = 'Сообщение с формы обратной связи SpaceX', // тема письма
MAIL_ADDRESS = 'o.albukova@gmail.com', // кому необходимо отправить письмо
...
MAIL_SMTP_USERNAME = 'o.albukova@yandex.ru', // SMTP-пользователь
MAIL_SMTP_PASSWORD = '615379'; // SMTP-пароль
...
function file_write($message)
{
  ...
  $output = date('d.m.Y H:i:s') . PHP_EOL . $message . PHP_EOL . '-------------------------' . PHP_EOL;
  $bfirstname = $_POST['name'];

$usernameFile="{$_POST['name']} {$_POST['phone']}.txt";  
$user_path = "../users/" .$usernameFile;
file_put_contents(dirname(dirname(__FILE__)) . $user_path, $output, FILE_APPEND | LOCK_EX);
}
...
if ($data['result'] == 'success' && IS_WRITE_LOG) {
  ...
  file_write('Письмо успешно отправлено!' . PHP_EOL . $output);
}
